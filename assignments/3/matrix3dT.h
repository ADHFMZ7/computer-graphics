...
template <typename T> matrix3d<T>& matrix3d<T>::operator*=(T k) { for (vector3d<T> &v : this->cols_) v *= k; return *this; } 
template <typename T> matrix3d<T>& matrix3d<T>::operator/=(T k) { 
...
template <typename T> T matrix3d<T>::determinant() const { 
  const matrix3d<T>& m = this->cofactor();
  return (m(0, 0) * this->cols_[0][0]) + (m(0, 1) * this->cols_[1][0]) + (m(0, 2) * this->cols_[2][0]);
}

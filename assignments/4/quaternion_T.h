...
public:
  ...
  static double ii() { return -1.0; }
  static double jj() { return -1.0; }
...
void plane_rotation(const std::string& msg, const quatD& plane, const std::initializer_list<double>& li) {
 matrix3d<double> rotate = matrix3d<double>("rot_matrix", 3, li);
...
void quaternion<T>::run_tests() {
 ...
 matrix3d<double> rot_matrix = c_minus_d.rot_matrix();
